...
test -f "$content_root/行程/2026-10 臺北/❂ 2026-10 臺北行程總覽.md"
test -f "$content_root/行程/2026-10 臺北/10-01（四）結婚登記與中正區.md"
test -f "$content_root/2026-10 臺北行程.base"
test ! -e "$content_root/.obsidian"
test ! -e "$content_root/.git"
test ! -e "$content_root/docs"
test ! -e "$content_root/docs/research"
...
grep -q '!\[\[2026-10 臺北行程.base\]\]' "$content_root/index.md"

expected_notes="$(find "$repo_root/景點" "$repo_root/區域" "$repo_root/捷運站" "$repo_root/書店" "$repo_root/行程" -type f -name '*.md' | wc -l | tr -d ' ')"
actual_notes="$(find "$content_root/景點" "$content_root/區域" "$content_root/捷運站" "$content_root/書店" "$content_root/行程" -type f -name '*.md' | wc -l | tr -d ' ')"
